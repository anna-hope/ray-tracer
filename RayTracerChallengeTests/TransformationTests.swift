...
final class TransformationTests: XCTestCase {
  ...
  func testRotatingAroundX() {
    ...
    let half_quarter = RTMatrix4x4.rotation_x(Double.pi / 4)
    let full_quarter = RTMatrix4x4.rotation_x(Double.pi / 2)
  ...
  func testRotatingAroundY() {
    ...
    let half_quarter = RTMatrix4x4.rotation_y(Double.pi / 4)
    let full_quarter = RTMatrix4x4.rotation_y(Double.pi / 2)
  ...
  func testRotatingAroundZ() {
    let p = RTPoint(x: 0, y: 1, z: 0)
    let half_quarter = RTMatrix4x4.rotation_z(Double.pi / 4)
    let full_quarter = RTMatrix4x4.rotation_z(Double.pi / 2)
    XCTAssertEqual(half_quarter * p, RTPoint(x: -sqrt_2_over_2, y: sqrt_2_over_2, z: 0))
    XCTAssertEqual(full_quarter * p, RTPoint(x: -1, y: 0, z: 0))
  }

  func testShearingXY() {
    let transform = RTMatrix4x4.shearing(1, 0, 0, 0, 0, 0)
    let p = RTPoint(x: 2, y: 3, z: 4)
    XCTAssertEqual(transform * p, RTPoint(x: 5, y: 3, z: 4))
  }

  func testShearingXZ() {
    let transform = RTMatrix4x4.shearing(0, 1, 0, 0, 0, 0)
    let p = RTPoint(x: 2, y: 3, z: 4)
    XCTAssertEqual(transform * p, RTPoint(x: 6, y: 3, z: 4))
  }

  func testShearingYX() {
    let transform = RTMatrix4x4.shearing(0, 0, 1, 0, 0, 0)
    let p = RTPoint(x: 2, y: 3, z: 4)
    XCTAssertEqual(transform * p, RTPoint(x: 2, y: 5, z: 4))
  }

  func testShearingYZ() {
    let transform = RTMatrix4x4.shearing(0, 0, 0, 1, 0, 0)
    let p = RTPoint(x: 2, y: 3, z: 4)
    XCTAssertEqual(transform * p, RTPoint(x: 2, y: 7, z: 4))
  }

  func testShearingZX() {
    let transform = RTMatrix4x4.shearing(0, 0, 0, 0, 1, 0)
    let p = RTPoint(x: 2, y: 3, z: 4)
    XCTAssertEqual(transform * p, RTPoint(x: 2, y: 3, z: 6))
  }

  func testShearingZY() {
    let transform = RTMatrix4x4.shearing(0, 0, 0, 0, 0, 1)
    let p = RTPoint(x: 2, y: 3, z: 4)
  ...
  func testIndividualTransformations() {
    ...
    let A = RTMatrix4x4.rotation_x(Double.pi / 2)
    let B = RTMatrix4x4.scaling(x: 5, y: 5, z: 5)
    let C = RTMatrix4x4.translation(x: 10, y: 5, z: 7)
  ...
  func testChainedTransformations() {
    let p = RTPoint(x: 1, y: 0, z: 1)
    let T = RTMatrix4x4.eye().rotate_x(Double.pi / 2).scale(x: 5, y: 5, z: 5)
